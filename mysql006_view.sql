...
-- Error Code: 1471. The target table emp_view3 of the INSERT is not insertable-into
insert into emp_view3 -- 집계함수 sum(salary)에 단순 숫자를 넣으려고 해서 오류가 나옴
VALUEs('p001',3500);
select*from emp2;
...
as
select e.id, e.name, e.loc , e.salary,e.code , e.bs , d.code
from emp2 e, dept1 d
where e.code = d.code;
...
select id, name, salary
from emp2 
where salary >= 3000 with check option;  -- 3000 이상만 넣을수 있음
select*from emp_view7;

-- error
insert into emp_view7
VALUEs('a011','야옹아',1000);
select*from emp2;
